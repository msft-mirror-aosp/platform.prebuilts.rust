...
fn default_hook(info: &PanicInfo<'_>) {
    ...
    cfg_if::cfg_if! {
        if #[cfg(not(target_os = "android"))] {
          if info.force_no_backtrace() {}
          else if panic_count::get_count() >= 2 { backtrace=BacktraceStyle::full()}
          else { backtrace=crate::panic::get_backtrace_style()};
        }
    };
    ...
    if let Ok(Some(local)) = try_set_output_capture(None) {
        cfg_if::cfg_if! {
            if #[cfg(not(target_os = "android"))] {write(&mut *local.lock().unwrap_or_else(|e| e.into_inner()));}
        };
